...
global {
    ...
    float distance_infection <- 2.0 #m; // Distance d'infection
    ...
    int max_personne <- 20; // Nombre maximal de personne dans le magasin
    int max_total_personne <- 200; // Nombre maximal de personne qui doivent entrer dans le magasin
    int malade_init <- 0; // Nombre de malade entré dans le magain
    int saine_init <- 0; // Nombre de personne saine entré dans le magain
    bool distanciation <- false; // Activer la distanciation
    bool masque <- false; // Activer le port de masque
    float distance_personne <- 3.0 #m; // Distance de distanciation
    float proba_infection init: 0.01;
    ...
    file my_csv_file <- csv_file("../includes/map_1.csv",",");
    ...
    reflex entree when: flip(0.25) and ((total_magasin < max_personne) and (total_personne < max_total_personne)) {
    	create species(personne) number: 1 {
		}
    }
